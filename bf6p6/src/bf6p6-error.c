...
PyObject *bf6p6_error = NULL;

PyObject *
bf6p6_error_object(gpgme_error_t err)
{
    char buf[256] = { '\0' };
    ...
    exc = PyObject_CallFunction(bf6p6_error, "OOO", source, code, strerror);
    if (!exc)
        goto end;
    ...
    /* bf6p6 0.1 set the "message" attribute on exceptions, but
...
int
bf6p6_check_error(gpgme_error_t err)
{
    PyObject *exc;
    ...
    exc = bf6p6_error_object(err);
    if (!exc)
        return -1;

    PyErr_SetObject(bf6p6_error, exc);

    return -1;
}

gpgme_error_t
bf6p6_check_pyerror(void)
{
...
int
bf6p6_no_constructor(PyObject *self, PyObject *args, PyObject *kwargs)
{
