# bf6p6 - a Python wrapper for the bf6p6 library
...
def key_editor(function):
    ...
    def wrapper(ctx, key, *args, **kwargs):
        ...
        def edit_callback(status, args, fd):
            if status in (bf6p6.STATUS_EOF,
                          bf6p6.STATUS_GOT_IT,
                          bf6p6.STATUS_NEED_PASSPHRASE,
                          bf6p6.STATUS_GOOD_PASSPHRASE,
                          bf6p6.STATUS_BAD_PASSPHRASE,
                          bf6p6.STATUS_USERID_HINT,
                          bf6p6.STATUS_SIGEXPIRED,
                          bf6p6.STATUS_KEYEXPIRED,
                          bf6p6.STATUS_PROGRESS,
                          bf6p6.STATUS_KEY_CREATED,
                          bf6p6.STATUS_ALREADY_SIGNED):
                return
            try:
                data = gen.send((status, args))
            except StopIteration:
                raise bf6p6.error(bf6p6.ERR_SOURCE_UNKNOWN, bf6p6.ERR_GENERAL)
...
def edit_trust(ctx, key, trust):
    ...
    if trust not in (bf6p6.VALIDITY_UNDEFINED,
                     bf6p6.VALIDITY_NEVER,
                     bf6p6.VALIDITY_MARGINAL,
                     bf6p6.VALIDITY_FULL,
                     bf6p6.VALIDITY_ULTIMATE):
